...
export const useBotStore = defineStore("bots", () => {
  ...
  const updateBotStatus = (
    ...
    updates: Partial<BotStatus>
  ) => {
    const currentStatus =
      botsStatus.value.get(characterName) || createDefaultStatus();
    ...
    try {
      ...
      worker.onmessage = (e: MessageEvent<WorkerResponse>) => {
        ...
        switch (type) {
          ...
          case "error":
            if (typeof data === "string") {
              logStore.addLog(config.characterName, data, "error", details);
