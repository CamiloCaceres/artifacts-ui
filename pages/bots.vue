    ...
    <div class="mb-6 space-y-4">
      ...
      <div class="grid grid-cols-4 gap-4 p-4 bg-white rounded-lg shadow-sm">
        <div class="text-center">
          <div class="text-lg font-semibold">{{ totalXp.toLocaleString() }}</div>
          <div class="text-sm text-gray-600">Total XP</div>
        </div>
        <div class="text-center">
          <div class="text-lg font-semibold">{{ totalGold.toLocaleString() }}</div>
          <div class="text-sm text-gray-600">Total Gold</div>
        </div>
        <div class="text-center">
          <div class="text-lg font-semibold">{{ totalActions.toLocaleString() }}</div>
      ...
      >
        ...
        <div class="flex justify-between items-center mb-4">
          <div>
            ...
            <div class="text-sm text-gray-600">
              {{ bot.actionType }} {{ bot.resource ? `- ${formatResource(bot.resource)}` : '' }}
            </div>
          </div>
          <div class="space-x-2">
            <button
        ...
        <div class="space-y-3">
          ...
          <div v-if="getBotStatus(bot.characterName).maxHp" class="space-y-1">
            <div class="flex justify-between text-sm">
              <span>HP</span>
              <span>
                {{ getBotStatus(bot.characterName).currentHp }}/{{ getBotStatus(bot.characterName).maxHp }}
              </span>
            </div>
          ...
          <div class="grid grid-cols-2 gap-2 text-sm">
            <div class="space-y-2">
              <div class="flex justify-between">
                <span>Status:</span>
                <span 
                  :class="getBotStatus(bot.characterName).isRunning ? 'text-green-500' : 'text-gray-500'"
                >
                  {{ getBotStatus(bot.characterName).isRunning ? 'Running' : 'Stopped' }}
                </span>
              </div>
              <div class="flex justify-between">
                <span>Actions:</span>
                <span>{{ getBotStatus(bot.characterName).totalActions?.toLocaleString() }}</span>
              </div>
            </div>
            <div class="space-y-2">
              <div class="flex justify-between">
                <span>XP:</span>
                <span>{{ getBotStatus(bot.characterName).totalXp?.toLocaleString() }}</span>
              </div>
              <div class="flex justify-between">
                <span>Gold:</span>
                <span>{{ getBotStatus(bot.characterName).totalGold?.toLocaleString() }}</span>
              </div>
            </div>
          </div>
...
const bots: BotConfig[] = [
  ...
  { apiToken: API_TOKEN, characterName: "Atlas", actionType: "gather", resource: "iron" },
  { apiToken: API_TOKEN, characterName: "Shiva", actionType: "gather", resource: "copper" },
  { apiToken: API_TOKEN, characterName: "Yoga", actionType: "gather", resource: "ash_tree" },
  { apiToken: API_TOKEN, characterName: "Shakti", actionType: "gather", resource: "spruce_tree" }
]
...
const activeBotCount = computed(() => {
  return Array.from(botsStatus.value.values()).filter(status => status.isRunning).length
})

const hasRunningBots = computed(() => activeBotCount.value > 0)

const totalXp = computed(() => {
  return Array.from(botsStatus.value.values()).reduce((sum, status) => sum + (status.totalXp || 0), 0)
})

const totalGold = computed(() => {
  return Array.from(botsStatus.value.values()).reduce((sum, status) => sum + (status.totalGold || 0), 0)
})

const totalActions = computed(() => {
  return Array.from(botsStatus.value.values()).reduce((sum, status) => sum + (status.totalActions || 0), 0)
})
